...
import { CardMedia, Grid, Card, CardContent, Typography, Rating, useTheme, CardActions, Button   } from '@mui/material';
  ...
  return (
        <Container sx={{py:8}} maxWidth="lg">
          <Grid container spacing={4}>
            { filteredProducts?.map(({title, id, image, price, description, rating})=>
            (<Grid item key={id} xs={12} sm={6} md={3}>
              <Card sx={{height:"100%", display:"flex", flexDirection:"column", padding:theme.spacing(2,0)}}>
                <CardMedia 
                  component="img" 
                  sx={{allignSelf:"center", width:theme.spacing(30), height:theme.spacing(30), objectFit:"contain", pt:theme.spacing()}} 
                  image={image} 
                  alt={title} />
                <CardContent>
                  <Typography 
                ...
                sx={{
                  alignSelf:"center",
                    ...
                    <Button variant="contained" onClick={()=>addProductToCart({title, id, image, price, description, rating})}>
                      <ShoppingCartSharp />
                      Add to Cart
                    </Button>
                </CardActions>
